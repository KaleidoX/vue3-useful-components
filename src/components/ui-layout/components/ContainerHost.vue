...
const activePanel = computed(() =>
  props.node.panels.find(p => p.id === props.node.activeId)
)
...
const showContent = computed(() => !collapsed.value && (props.node.type !== 'dock' || props.node.activeId !== ''))
</script>
...
<template>
  <div class="h-full w-full flex flex-col overflow-hidden bg-white dark:bg-[#0d1117]">
    ...
    <div v-if="node.type === 'dock'" class="flex shrink-0 flex-col select-none">
      <div v-for="panel in node.panels" :key="panel.id"
    ...
    <!-- Stack header -->
    <div v-if="node.type === 'stack' && node.panels.length > 0"
      ...
      <template v-for="(panel, i) in node.panels" :key="panel.id">
        <button v-if="i > 0" class="border-none bg-none p-0 text-slate-300 dark:text-[#30363d]">›</button>
        <button class="h-[20px] cursor-pointer rounded border-none px-2 text-[11px] transition-colors"
          :class="panel.id === node.activeId ? 'bg-blue-500 text-white' : 'bg-transparent text-slate-500 hover:bg-slate-200 dark:text-[#8b949e] dark:hover:bg-white/5'"
    ...
    <div v-if="showContent" class="relative flex-1 overflow-hidden">
      ...
      <div v-else class="h-full w-full flex items-center justify-center text-sm text-slate-300 dark:text-[#484f58]" />
    </div>
  </div>
</template>
