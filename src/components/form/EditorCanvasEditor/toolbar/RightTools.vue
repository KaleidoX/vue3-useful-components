  ...
  <div class="menu-right">
    ...
    <div title="页面模式" @click="toggleDropdown('pm')">
      ...
      <div v-show="ds.pm" class="menu-options" style="right:0;left:auto;" @click.stop>
        <li @click="exec('executePageMode', 'page'); ds.pm = false">分页模式</li>
        <li @click="exec('executePageMode', 'continuous'); ds.pm = false">连续模式</li>
      </div>
    </div>
...
function cycleMode() { modeIndex = (modeIndex + 1) % modes.length; mode.value = modes[modeIndex]; exec('executeMode', modeKeys[modeIndex]) }
