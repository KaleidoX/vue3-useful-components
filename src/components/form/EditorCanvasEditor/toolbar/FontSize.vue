  ...
  <div class="menu-item">
    <div class="menu-item__font" @click="toggleDropdown('font')">
      ...
      <div v-show="ds.font" class="menu-options" @click.stop>
        <li v-for="f in fontList" :key="f" :class="{ active: currentFont === f }" @click="applyFont(f)">{{ f }}</li>
      </div>
    </div>
    <div class="menu-item__size" @click="toggleDropdown('size')">
      <span>{{ currentSize }}</span>
      <div v-show="ds.size" class="menu-options" @click.stop>
        <li v-for="s in sizeList" :key="s" :class="{ active: currentSize === s }" @click="applySize(s)">{{ s }}</li>
      </div>
    </div>
...
const fontList = ['微软雅黑', '宋体', '楷体', '仿宋', '黑体', 'Arial', 'Times New Roman', 'Courier New']
const sizeList = [12, 14, 16, 18, 20, 24, 28, 32, 36, 42, 48, 56, 64, 72]

function toggleDropdown(k: string) { ds[k] = !ds[k] }
function applyFont(f: string) { exec('executeFont', f); currentFont.value = f; ds.font = false }
function applySize(s: number) { exec('executeSize', s); currentSize.value = s; ds.size = false }
</script>
