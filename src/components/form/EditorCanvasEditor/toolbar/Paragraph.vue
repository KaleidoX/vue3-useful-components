<template>
  <div class="menu-item">
    ...
    <div title="两端对齐" @click="exec('executeRowFlex', 3)"><i class="i-lucide:align-justify" /></div>
    <div title="分散对齐" @click="exec('executeRowFlex', 4)"><i class="i-lucide:align-center-vertical" /></div>
    <div class="menu-item__row-margin" @click="toggleDropdown('rowmargin')">
      <span>行距</span>
      <div v-show="ds.rowmargin" class="menu-options" @click.stop>
        <li v-for="r in rowMarginList" :key="r.value" @click="exec('executeRowMargin', r.value); ds.rowmargin = false">{{ r.label }}</li>
      </div>
    </div>
    <div class="menu-item__list" @click="toggleDropdown('list')">
      <span>列表</span>
      <div v-show="ds.list" class="menu-options" @click.stop>
        <li @click="exec('executeList', 0, null); ds.list = false">无序列表</li>
        <li @click="exec('executeList', 1, null); ds.list = false">有序列表</li>
      </div>
    </div>
  </div>
</template>
...
const titleList = [
  ...
  { key: 'h1', level: '1', label: '标题 1' }, { key: 'h2', level: '2', label: '标题 2' },
  { key: 'h3', level: '3', label: '标题 3' }, { key: 'h4', level: '4', label: '标题 4' },
  { key: 'h5', level: '5', label: '标题 5' }, { key: 'h6', level: '6', label: '标题 6' },
]
const rowMarginList = [
  { value: 1, label: '1.0' }, { value: 1.25, label: '1.25' }, { value: 1.5, label: '1.5' },
  { value: 1.75, label: '1.75' }, { value: 2, label: '2.0' }, { value: 2.5, label: '2.5' }, { value: 3, label: '3.0' },
]
