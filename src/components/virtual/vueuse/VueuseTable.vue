...
<template>
  <div v-bind="containerProps" class="h-128 overflow-y-auto border border-gray-200 rounded">
    <div class="sticky top-0 z-1 grid grid-cols-7 bg-gray-100 px-3 text-left text-sm text-gray-600 font-medium"
    ...
    <div v-bind="wrapperProps">
      <div
        ...
        :style="{ height: contentType === 'dynamic' ? `${row.id % 3 === 0 ? '70' : row.id % 3 === 1 ? '45' : '90'}px` : '45px', alignItems: 'center' }"
      >
        <span>{{ row.id }}</span>

        <template v-if="contentType === 'editable'">
          <input class="w-4/5 border border-gray-300 rounded px-1 py-0.5 text-xs" :value="row.col0"
        ...
        <template v-if="contentType === 'el-checkbox'">
          <ElCheckbox :model-value="row.col1" size="small"
            @change="(v) => emit('update:cell', row.id, 'col1', v)" />
        </template>
        <template v-else-if="contentType === 'native-checkbox'">
          <input type="checkbox" :checked="row.col1"
            @change="emit('update:cell', row.id, 'col1', ($event.target as HTMLInputElement).checked)" />
        </template>
        <span v-else :class="row.col1 ? 'text-green-600' : 'text-red-500'">{{ row.col1 ? '是' : '否' }}</span>

        <template v-if="contentType === 'editable'">
          <input class="w-4/5 border border-gray-300 rounded px-1 py-0.5 text-xs" :value="row.col2"
            @input="emit('update:cell', row.id, 'col2', ($event.target as HTMLInputElement).value)" />
        </template>
        <span v-else class="truncate">{{ row.col2 }}</span>

        <template v-if="contentType === 'editable'">
          <input class="w-4/5 border border-gray-300 rounded px-1 py-0.5 text-right text-xs" type="number" :value="row.col3"
            @input="emit('update:cell', row.id, 'col3', Number(($event.target as HTMLInputElement).value))" />
        </template>
        <span v-else class="text-right font-mono">{{ row.col3.toLocaleString() }}</span>

        <template v-if="contentType === 'editable'">
          <input class="w-4/5 border border-gray-300 rounded px-1 py-0.5 text-xs" :value="row.col4"
        ...
        <div class="flex items-center gap-1">
          <template v-if="contentType === 'complex'">
            <span class="inline-block h-5 w-5 rounded-full bg-blue-200 text-center text-xs leading-5">{{ row.id % 3 }}</span>
